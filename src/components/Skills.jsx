...
export default function Skills() {
  ...
  return (
    <section id="skills" className="py-10 sm:py-16 md:py-20 relative overflow-hidden bg-slate-950/40">
      {/* Background glow */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          {tabs.map((tab) => {
            ...
            return (
              <button
                ...
                className={`px-3.5 sm:px-4 py-1.5 sm:py-2 rounded-[10px] text-[11px] sm:text-xs font-bold uppercase tracking-wider transition-all duration-200 flex items-center gap-1.5 sm:gap-2 ${
                  isActive
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
          {displayedSkills.map((skill, idx) => (
            <div
              key={idx}
              className="glass-card p-3.5 sm:p-4 rounded-[14px] flex flex-col items-center text-center relative overflow-hidden group border border-slate-800/80 hover:border-brand-cyan/40 transition-all duration-200"
            >
              ...
              {/* Icon Frame */}
              <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-[10px] bg-slate-900/90 border border-slate-800 flex items-center justify-center text-xl sm:text-2xl mb-2 sm:mb-3 shadow-inner">
              ...
              {/* Badge Level */}
              <span className="text-[8px] sm:text-[9px] font-mono px-2 py-0.5 rounded-[6px] bg-slate-800/90 text-slate-300 mb-2 sm:mb-3 border border-slate-700/60 uppercase">
                {skill.badge}
              </span>
              ...
              <div className="flex justify-between items-center w-full mt-1 px-0.5 text-[8px] sm:text-[9px] font-mono text-slate-500 uppercase">
        ...
        <div className="mt-8 sm:mt-12 p-4 sm:p-6 rounded-[14px] bg-dark-900/60 border border-slate-800/80 flex flex-wrap items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            <div className="p-2 rounded-[8px] bg-emerald-500/10 border border-emerald-500/30 text-emerald-400">
              <CheckCircle2 className="w-5 h-5" />
            </div>
            <div>
              <div className="text-xs sm:text-sm font-bold text-white uppercase tracking-wide">CONTINUOUS EVOLUTION</div>
              <div className="text-[11px] sm:text-xs text-slate-400">Always learning modern libraries, TypeScript, Next.js App Router, and AI tooling.</div>
            </div>
          </div>

          <a
            href="#projects"
            className="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-brand-cyan hover:text-cyan-300 flex items-center gap-1.5 ml-auto"
          >

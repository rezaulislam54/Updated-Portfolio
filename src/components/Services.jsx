      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-8">
          {services.map((svc) => (
            <div
              key={svc.id}
              className="glass-card p-5 sm:p-8 rounded-[14px] border border-slate-800/80 hover:border-violet-500/40 group flex flex-col justify-between transition-all duration-200"
            >
              <div>
                {/* Service Icon */}
                <div className="w-11 h-11 sm:w-12 sm:h-12 rounded-[10px] bg-slate-900/90 border border-slate-800 flex items-center justify-center mb-4 sm:mb-5 shadow-inner">
                  {serviceIcons[svc.icon]}
                </div>

                {/* Title */}
                <h3 className="text-base sm:text-xl font-bold text-white mb-2 sm:mb-3 group-hover:text-violet-300 transition-colors uppercase tracking-wide">
                  {svc.title}
                </h3>
              ...
              {/* Order/Contact CTA */}
              <div className="pt-3.5 sm:pt-4 border-t border-slate-800/60">
                <a
                  href="#contact"

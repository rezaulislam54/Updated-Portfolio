...
export default function Projects() {
  ...
  return (
    <section id="projects" className="py-10 sm:py-16 md:py-20 relative overflow-hidden">
      {/* Background glow */}
          ...
          {categories.map((cat) => {
            ...
            return (
              <button
                ...
                className={`px-3.5 sm:px-4 py-1.5 sm:py-2 rounded-[10px] text-[11px] sm:text-xs font-bold uppercase tracking-wider transition-all duration-200 ${
                  isActive
          ...
          {filteredProjects.map((project) => (
            ...
            >
              ...
              <div className="p-4 sm:p-6 flex flex-col flex-grow justify-between">
                <div>
                  <h3 className="text-base sm:text-xl font-bold text-white group-hover:text-cyan-400 transition-colors mb-1.5 flex items-center justify-between uppercase tracking-wide">
                    <span>{project.title}</span>
                    <FolderGit2 className="w-4 h-4 text-slate-500 group-hover:text-brand-cyan transition-colors" />
                  </h3>
                  <p className="text-[11px] sm:text-xs text-slate-400 font-mono mb-2.5 sm:mb-3 uppercase tracking-wider">
                    {project.tagline}
                  </p>
                ...
                <div>
                  ...
                  <div className="flex items-center justify-between pt-3 border-t border-slate-800/80">
                    <button
                      onClick={() => setSelectedProject(project)}
                      className="text-[11px] sm:text-xs font-bold uppercase tracking-wider text-slate-400 hover:text-white flex items-center gap-1.5 transition-colors"
                    >
                    ...
                    <div className="flex items-center gap-2">
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="p-1.5 sm:p-2 rounded-[8px] bg-slate-800/80 text-slate-300 hover:text-white hover:bg-slate-700 border border-slate-700/60 transition-all"
                      ...
                      <a
                        ...
                        target="_blank"
                        rel="noreferrer"
                        className="px-3 sm:px-3.5 py-1.5 rounded-[8px] text-[11px] sm:text-xs font-bold uppercase tracking-wider text-white bg-gradient-to-r from-brand-cyan to-brand-violet hover:opacity-90 transition-all flex items-center gap-1 shadow-md shadow-cyan-500/20"
                      >
                        <span>LIVE DEMO</span>
